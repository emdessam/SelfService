package hrss.intercom.eg.hrselfservice.offline.json.parser;
...
public class StaticJSONParser {
    ...
    public static String checkURI(String pUri, String mMethod) {
        ...
        //  ana me7tag methode te3ml check 3la kol request we terg3 el response el crossponding leeh
        Log.e("RmMethod+pUri", mMethod + pUri + "furtui");
        switch (mMethod + pUri) {

            case ServerConfig.METHOD_POST + ServerConfig.LOGIN_PATH:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_POST + "_" + ServerConfig.LOGIN_PATH + ".json"));
                Log.e("RmResponse", response);
                break;
            case ServerConfig.METHOD_GET + ServerConfig.VAC_BALANCE:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_GET + "_" + ServerConfig.VAC_BALANCE + ".json"));
            ...
            case ServerConfig.METHOD_GET + ServerConfig.PROFILE_DETAIL:
                ...
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_GET + "_" + ServerConfig.PROFILE_DETAIL + ".json"));
                Log.e("VBResponse", response);

                break;
            case ServerConfig.METHOD_GET + ServerConfig.PENDING_REQUEST:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_GET + "_" + ServerConfig.PENDING_REQUEST + ".json"));
                Log.e("VBResponse", response);

                break;
            case ServerConfig.METHOD_GET + ServerConfig.COMPENSATION_HISTORY:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_GET + "_" + ServerConfig.COMPENSATION_HISTORY + ".json"));
                Log.e("VBResponse", response);

                break;
            case ServerConfig.METHOD_GET + ServerConfig.PERMISSION_HISTORY:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_GET + "_" + ServerConfig.PERMISSION_HISTORY + ".json"));
                Log.e("VBResponse", response);

                break;
            case ServerConfig.METHOD_GET + ServerConfig.MISSION_HISTORY:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_GET + "_" + ServerConfig.MISSION_HISTORY + ".json"));
                Log.e("VBResponse", response);

                break;
            case ServerConfig.METHOD_GET + ServerConfig.VACATION_HISTORY:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_GET + "_" + ServerConfig.VACATION_HISTORY + ".json"));
                Log.e("VBResponse", response);

                break;
            case ServerConfig.METHOD_POST + ServerConfig.VACATION_REQUEST:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_POST + "_" + ServerConfig.VACATION_REQUEST + ".json"));
                Log.e("RmResponse", response);
                break;
            case ServerConfig.METHOD_POST + ServerConfig.PERMISSION_REQUEST:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_POST + "_" + ServerConfig.PERMISSION_REQUEST + ".json"));
                Log.e("RmResponse", response);
                break;
            case ServerConfig.METHOD_POST + ServerConfig.COMPENSATION_REQUEST:
                response = loadJSONFromAsset(renamJsonFile(ServerConfig.METHOD_POST + "_" + ServerConfig.COMPENSATION_REQUEST + ".json"));
                Log.e("RmResponse", response);
                break;
...
//        }
    ...
    public static String renamJsonFile(String fileName) {
